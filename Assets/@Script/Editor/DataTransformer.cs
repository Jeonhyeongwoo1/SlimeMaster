...
public class DataTransformer : EditorWindow
{
    ...
    static Dictionary<int, List<WaveData>> ParseWaveData(string filename)
    {
        ...
        for (int y = 1; y < lines.Length; y++)
        {
            ...
            waveData.WaveType = ConvertValue<WaveType>(row[i++]);
    ...
    static void ParseMaterialData(string filename)
    {
        ...
        for (int y = 1; y < lines.Length; y++)
        {
            ...
            material.MaterialType = ConvertValue<MaterialType>(row[i++]);
            material.MaterialGrade = ConvertValue<MaterialGrade>(row[i++]);
    ...
    static void ParseDropItemData(string filename)
    {
        ...
        for (int y = 1; y < lines.Length; y++)
        {
            ...
            dropItem.DropItemType = ConvertValue<DropableItemType>(row[i++]);
    ...
    static void ParseDefaultUserData(string filename)
    {
        DefaultUserDataLoader loader = new();
        #region ExcelData
        string[] lines = File.ReadAllText($"{Application.dataPath}/@Resources/Data/Excel/{filename}Data.csv").Split("\n");
        ...
        for (int y = 1; y < lines.Length; y++)
        {
            ...
            DefaultUserData item = new DefaultUserData();
            item.itemId = ConvertValue<int>(row[i++]);
            item.itemName = ConvertValue<string>(row[i++]);
            item.itemValue = ConvertValue<int>(row[i++]);

            loader.defaultUserDataList.Add(item);
        }
        #endregion
